...
public class Delete_Medical_Reports_Doctor {

	static Logger log = Logger.getLogger(Delete_Medical_Reports_Doctor.class.getName());
	...
	By cancal_btn = By.id("btn_cancel");
	...
	public void Report_added_by_Doctor() throws InterruptedException {
		...
		for (WebElement wenm : nm) {
			...
			do
			{
				System.out.println("in inner do");
				List<WebElement> del = driver.findElements(del_btn);
				System.out.println("del.size()  :"+del.size());
				
				for (WebElement Record_we : del)
				{
